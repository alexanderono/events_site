...
<html>
  ...
  <div id="bigdamnwrapper">
    ...
    <div class="blogpost">
      <?php
        $thumbnail_id = isset($_REQUEST['thumbnail_id']) ? $_REQUEST['thumbnail_id'] : null;
        $event_id = isset($_REQUEST['event_id']) ? $_REQUEST['event_id'] : null;

        $insertSQL = "DELETE thumbnails FROM thumbnails WHERE thumbnail_id = $thumbnail_id";
	      $success = mysqli_query($conn, $insertSQL) or die(mysqli_error($conn));
        if($success === true){
          echo "Success, thumbnail deleted.";
        }else{
          echo "Error, thumbnail not deleted.";
        }
