...
<html>
  <head>
    <meta charset="utf-8">
    <title>Upload image - The Great North East</title>
  ...
  <div id="bigdamnwrapper">
    ...
    <div class="blogpost">
      ...
      $youtube_url = isset($_REQUEST['youtube_url']) ? $_REQUEST['youtube_url'] : null;

      if($youtube_url === null){
        die("<p>No URL entered.</p> <a href=\"staff.php\">Go back</a>");
      }

      $sql = "INSERT INTO event_urls (event_url, event_ref) VALUES ('$youtube_url','$event_id') ";
      $success = mysqli_query($conn, $sql) or die(mysqli_error($conn));
      if($success === true){
       echo "<p>Youtube URL uploaded!</p><br /><a><p><a href=\"editMedia.php?event_id=$event_id\">Click here to go back</a></p>";
      }else{
       echo "<p>Error, URL not uploaded.</p><br /><a><p><a href=\"editMedia.php?event_id=$event_id\">Click here to go back</a></p>";
      }
      mysqli_close($conn);
